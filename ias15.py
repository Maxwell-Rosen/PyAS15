...
def pushIAS15(derivative, startingPoint, dt, lastB=None):
    ''' Input:
    derivative - the derivative of the function to integrate. Right hand side of the ODE
    startingPoint - the starting point of the integration. Tupple of (R,Z)
    Output:
    newPosition - the new position after the integration. Tupple of (R,Z)'''
    ...
    for i in range(6):
        y = calculateNodePositions(B, startingPoint, dt, hp = h)
        F = calculateDerivatives(derivative, y)
        G = calculateGFromF(F)
        B = calculateB(G)

    newPosition = calculateNewPosition(B, startingPoint, dt)
    return newPosition

def calculateNodePositions(B, startingPoint,dt, hp = h):
    F1 = calculateDerivatives(startingPoint)
    Rh = np.zeros(len(hp))
    Zh = np.zeros(len(hp))
    for n in range(len(hp)):
        Rh[n] = startingPoint[0] + hp[n]*dt*(F1 + hp[n]/2*( B.R[0] + hp[n]*2/3*( B.R[1] + hp[n]*3/4*( B.R[2] + hp[n]*4/5*( B.R[3] + hp[n]*5/6*( B.R[4] + hp[n]*6/7*( B.R[5] + hp[n]*7/8*B.R[6])))))))
        Zh[n] = startingPoint[1] + hp[n]*dt*(F1 + hp[n]/2*( B.Z[0] + hp[n]*2/3*( B.Z[1] + hp[n]*3/4*( B.Z[2] + hp[n]*4/5*( B.Z[3] + hp[n]*5/6*( B.Z[4] + hp[n]*6/7*( B.Z[5] + hp[n]*7/8*B.Z[6])))))))
    return (Rh,Zh)

def calculateNewPosition(B, startingPoint, dt):
    newPosition = calculateNodePositions(B, startingPoint, dt, positions = 1)
    return newPosition

def calculateDerivatives(derivative, y):
    R = y[0]
    Z = y[1]
    F = np.zeros(len(R))
    for i in range(len(R)):
        F[i] = derivative(R[i],Z[i])
    return F
